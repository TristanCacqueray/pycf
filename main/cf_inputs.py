...
class wave_reader:
    def __init__(self, infile, frames_per_buffer=4410):
        ...
        print("fmt %d, %d Hz stream of %d size (%d channel)" % (
                sampwidth, rate, frames_per_buffer, channels))
        if channels > 1:
...
class audio_process:
    def __init__(self, fps=25, input_fd=None, output_device_index=None):
        self.q = Queue(4096 * 4096)
        self.rate = 44100     # hardcoded 44100 Hz sample rate
        if self.rate / fps != self.rate / fps:
            raise RuntimeError("Frame per second must be a multiple of %d" %
                               self.rate)
    ...
    def get_device_name(self, idx):
        if not idx:
            return "default"
        return self.p.get_device_info_by_index(int(idx))['name']
    ...
    def open_stream(self, input=False, output=False):
        return self.p.open(
                ...
                input_device_index=int(self.input_fd),
...
#                print("The queue is full!")
    ...
    def recv(self, render_video=None):
        if not render_video and self.q.qsize() > 3:
            print("Processing too slow - %d frames late, flushing..." %
                  self.q.qsize())
            try:
                while True:
                    self.q.get(False)
