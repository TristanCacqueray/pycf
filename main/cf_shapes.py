...
class spirals(shape):
    ...
    def gen_spirals(self):
        ...
        color = red
        for i in range(int(self.params["spikes"].value)):
            self.shape.append(
                    "100 * {y %.3f x %.3f rotate %.3f size 0.98 h 0.5 z 1} "
                    "CIRCLE {h 10 sat 0.9 b 1 s 3}" % (
    ...
    def gen_circles(self):
        ...
        for idx in range(int(self.params["circles"].value)):
            i = 40 + 2.5 * idx
            this_rot = cur_rot + idx * ang_step
            if idx & 1:
                this_rot = -1 * this_rot
            self.shape.append(
                    "%d * {y 1 r 15 h 8 } TRIANGLE "
                    "{y %f x %f s 2 sat 1 b 1 h 60}" % (
                            clen,
                            i * sin(this_rot),
                            i * cos(this_rot)
                    )
    ...
    def process(self):
        ...
        if new_bg < cur_bg:
            new_bg = cur_bg - 0.5
        elif new_bg - cur_bg < 5:
            new_bg = cur_bg
        self.state["bg_hue"] = new_bg
        self.shape[1] = "background { h %.3f sat 0.8 b %.3f}" % (
                new_bg, -0.5 + abs(230 - new_bg)/230.0)
...
class twofirs(shape):
    ...
    def process(self):
        ...
        if alow < self.last_low:
            alow = self.last_low - 0.02
        if amid < self.last_mid:
            amid = self.last_mid - 0.02
        if ahigh < self.last_high:
            ahigh = self.last_high - 0.01
        self.last_low, self.last_mid, self.last_high = alow, amid, ahigh
        size = self.params["size"].value
        num = self.params["num"].value
        for idx in range(self.params["spikes"].value):
            ...
            for x, r, y, h in ((ahigh, alow, 1, 60), (-alow, ahigh, -1, 219)):
                ...
                y *= side
                self.shape.append(
                    "%d * { x %.3f y %.3f r %.3f z 1 s 0.99 h -0.4 } "
                    "TRIANGLE { hue %d b %f sat 1 s %.3f }" % (
...
class drops(shape):
    ...
    class drop:
        def __init__(self, pos, color, size, shape, vec):
            self.pos = pos
            self.color, self.size, self.shape, self.age = color, size, shape, 1
            self.vec = vec
...
#        print len(self.fft), N.max(self.fft)
        ...
        for i in range(int(freqs)):
            if self.fft[i] > (limit):  # -i/float(freqs)*0.4
                ...
                self.drops.append(
                    drops.drop(
                        "y %.3f x %.3f r 0" % (-60 + i / float(freqs) * 120,
                                               pos),
                        "h %d b 1 sat 1" % (30 - (i/float(freqs) * 30) -
                                            (abs(pos) / 60.0 * 30)),
