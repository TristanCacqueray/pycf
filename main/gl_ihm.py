...
class scene:
    def pygame_init(self):
        self.surface = pygame.display.set_mode(
                self.dim, OPENGL | DOUBLEBUF | HWSURFACE)
...
#         glViewport(0, 0, self.dim[0], self.dim[1])
    ...
    def do(self, shape, output=None):
        ...
        if self.travelling_rot and self.age > 0:
            shape.center[self.travelling_rot[0]] += \
                self.travelling_rot[1] * (self.age / 131.)
            if self.age == 1:
                self.travelling_rot = None
        if self.travelling_pos and self.age > 0:
            shape.eye[self.travelling_pos[0]] += \
                self.travelling_pos[1] * (self.age / 131.)
            if self.age == 1:
                self.travelling_pos = None
        self.age -= 1
